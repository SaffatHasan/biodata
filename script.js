...
window.onload = function () {
    ...
    fetch("data.yml", { cache: "no-cache" })
        .then(response => response.text())
        .then(rawYaml => jsyaml.load(rawYaml))
        .then(data => {
            console.log(data);
            console.log(getFocusedName());
            data = tagPerson(data, getFocusedName());
            chart.load(data);
        });
};

function getFocusedName() {
    const defaultName = "saffat hasan";
    const urlSearchParams = new URLSearchParams(window.location.search);
    const params = Object.fromEntries(urlSearchParams.entries());
    return params["focus"] || defaultName;
}

function tagPerson(data, person) {
    if (person == null) {
        return data;
    }

    const personIndex = data.findIndex( x => x.name.toLowerCase() === person);

    if (personIndex === -1) {
        return data;
    }

    data[personIndex].tags = ["blue"];
    return data;
}